...
if [ "$cygwin" = true -o "$msys" = true ] ; then
    ...
    CLASSPATH="$CLASSPATH:$ROOTDIR/gradle/wrapper/gradle-wrapper.jar"
    ...
    for arg in "$@" ; do
        ...
        if [ $CHECK -eq 1 ] ; then ### Added a condition
            ARG=`echo "$arg"|sed 's/^-//'` ### Defines what to strip
            eval `echo args$i`=`cygpath --path --mixed "$ARG"` ### Reassemble individual args
            i=`expr $i + 1`
        else
            eval `echo args$i`="\"$arg\"" ### Reassemble individual args
            i=`expr $i + 1`
        fi
    done
    case $i in
